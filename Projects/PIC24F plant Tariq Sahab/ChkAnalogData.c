...
void SetATLFlags(short,unsigned char) ;    
...
void SetATLFlags( short MiniNum,unsigned char Modifier)
{
  
  CSPATLMOD[MiniNum] = Modifier  & 0xFF; 
  CSPATLReq[MiniNum] = 1 ;
  PORTE = 0x01;
}
...
{ 
  ...
  for(MiniNum=0;MiniNum<12;MiniNum++){
    if(((CSPCStatus[MiniNum]& 0x03)==0x03) ||((CSPCStatus[MiniNum]& 0x06)==0x06)||((CSPCStatus[MiniNum]& 0x05)==0x05)){
      Form_VLV_MCFG(MVLVCMD[MiniNum],MiniNum);
      PORTE=0x10;
      if(C12DAT.MCFGDT[MiniNum] > 1) {  
        SetATLFlags(MiniNum,REC_MF) ;    
        CLRBITchar(MinChanBMP,MiniNum) ;
        CSPPStatus[MiniNum] = CSPCStatus[MiniNum]  ;
      }
      else SetCSPILC_SE();
    }
    else if(((CSPCStatus[MiniNum]& 0x18)==0x18)||((CSPCStatus[MiniNum]& 0x30)==0x30)||((CSPCStatus[MiniNum]& 0x28)==0x28)){
      Form_VLV_MCFG(MVLVCMD[MiniNum],MiniNum);
      PORTE=0x08;
      if(C12DAT.MCFGDT[MiniNum] > 2) {  
        SetATLFlags(MiniNum,VEV_MF) ;       
        CLRBITchar(MinChanBMP,MiniNum) ;
        CSPPStatus[MiniNum] = CSPCStatus[MiniNum]  ;
      }
      else SetCSPILC_SE();
...
int SetCSPCurStsAcPreSts(short MinCasNo)
{
  short BNo ;
