...
void ProcessValves(void)
{
  ...
  for (C12CasNo=0; C12CasNo<=1; C12CasNo++) {
    ProcValData(C12CasNo);                 	// Process Mini Cascade Valves Data
  }
  //SendCommand();	                    // Send Command to Electronics
  //ValSendSE();  	                    // Send ATL/ILC S.E.
}

int ProcValData(int CasNo1)
{
   //unsigned char *PrvPtr, *CurPtr;
   unsigned char VSERStat ; 
...
//   VpdChg = Chk_VLV_VPD(CasNo); 	// Check ang change in valve vpd
   if (CasNo1==MainCas) VSERStat = C12PUSDT.ESERDT;
   else                VSERStat = C12PUSDT.MSERDT[CasNo1];
