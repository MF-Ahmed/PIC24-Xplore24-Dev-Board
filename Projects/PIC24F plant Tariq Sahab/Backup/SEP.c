...
void SendCPS_PDR()
{
  ...
  for(num=0; num<96; num++)
    t2Buf[18+num] = *cptr++;
  cptr=(unsigned char*)C12Ptr->EVLVCDT;
  for(num=0; num<4; num++)
    t2Buf[114+num] = *cptr++;
  cptr=(unsigned char*)C12Ptr->MVLVCDT;
  for(num=0; num<24; num++)
		...
		if(TSTBITchar(&x[1],i)){
		  if(i==12){									// Main Cascade
			if(x[3]==CFG_MF){							// CFG_MF
			  ...
			  for(j=0; j<2; j++) {
				if(CCFGData[0]) {
				  if(j==1) {CVLVData[j*2]=PVLVData[j*2];}
				  else CVLVData[j]=EOpenValCfg[CCFGData[j]-1];
				}
			  }
			}
...
void UpDateDRData()
{
  ...
  if(DR==0x82) {
    ...
    //ProMacFrqData();
    ProcDigitalData() ;		// Waseem Akhtar	ProcMsECsPccData()
...
void ProcessDRAck()
{
  if((DR==DRAck) && DRTmoCnt>0) {
	...
	if(DRAck==0x7F) {ProcessCMP_HWC_PCC_SE(); DRInProgress=0;DRAckRecd=DRDataRecd=0;DRDelay=2;}
	if(DRAck==0x7E) {ProcessMCVCmd_SE(); DRInProgress=0;DRAckRecd=DRDataRecd=0;DRDelay=2;}
  }
  else {
