...
void ProcessValves(void)
{
  ...
  for (C12CasNo=0; C12CasNo<=11; C12CasNo++) {
...
int ProcValData(int CasNo)
{
   ...
   int VlvStsChg=0, F_VlvStsChg=0, vflag=0, CNo=0;
   int VcdVsdFlag=0, VpdChg=0, ByteNo, CFGDefined=0;
...
//   VpdChg = Chk_VLV_VPD(CasNo); 	// Check ang change in valve vpd
   if (CasNo==MainCas) VSERStat = C12PUSDT.ESERDT;
   else                VSERStat = C12PUSDT.MSERDT[CasNo];
